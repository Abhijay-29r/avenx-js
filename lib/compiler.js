...
import { parseRouteTable } from './compiler/atlas/routes.js';
...
import { createSessionFingerprint } from './compiler/incrementalCache.js';
import {
  bridgeModule,
  componentModule,
  devtoolsModule,
...
class AvenxCompiler {
  ...
  constructor(options = {}) {
    ...
    /**
     * Whether this build may reuse cached compilation products.
     *
     * Two gates, both of which have to be open. `incremental` in
     * avenx.config.json is the project's opt-in, and it is off by default
     * because correctness here is demonstrated by a test rather than assumed.
     * `watch` is passed by the long-lived commands -- `serve`, `watch`,
     * `check --watch` -- and by nothing else, which is what keeps `avenx build`
     * a cold build no matter what the config says: a production artifact must
     * never depend on the state of an in-memory cache.
     * @type {boolean}
     */
    this.incremental = config.incremental === true && options.watch === true;
  ...
  build() {
    ...
    this.componentParser.setRouteParams(this.collectRouteParams());
    ...
    // Opened here rather than in the constructor: the fingerprint covers the
    // project's configuration, and the cross-unit half of the key is measured
    // from the bridges, names and route parameters that were just collected.
    this.componentParser.setIncrementalSession(
      this.incremental
        ? createSessionFingerprint({ rootDir: this.rootDir, config: this.config, publicEnv: this.publicEnv })
        : null,
    );
  ...
  analyze(options = {}) {
    ...
    try {
      ...
      phase('routeParams', () => this.componentParser.setRouteParams(this.collectRouteParams()));
      this.componentParser.setIncrementalSession(
        this.incremental
          ? createSessionFingerprint({ rootDir: this.rootDir, config: this.config, publicEnv: this.publicEnv })
          : null,
  ...
  /**
   * The route parameters each page can receive, keyed by page name.
   *
   * A pattern such as `'/profile/:id'` means the router sets `id` on the page's
   * state before it renders -- `mountPage` assigns every entry of `params`
   * straight into `instance.state`. The template validator had no way to know
   * that, so `{{ id }}` on a page reached by a parameterised route was reported
   * as an undeclared reference (AVX_W03). Route parameters are documented in
   * the routing tutorial, so following that tutorial made `avenx check` exit 1
   * on correct code.
   *
   * `query` is included for every routed page, because `matchRoute` sets
   * `params.query` whenever the URL carries a query string, and any route can
   * be visited with one.
   *
   * Read from `main.app.js` with the same parser Atlas uses, so the validator
   * and the model cannot disagree about what a route declares. A project whose
   * route table the parser cannot read contributes nothing here and keeps the
   * previous behaviour.
   * @returns {Map<string, Set<string>>} Parameter names by page name.
   * @private
   */
  collectRouteParams() {
    ...
    const mainFile = path.join(this.srcDir, 'main.app.js');
    if (!fs.existsSync(mainFile)) return byPage;

    let source;
    try {
      source = fs.readFileSync(mainFile, 'utf-8');
    } catch {
      return byPage;
    }
    ...
    for (const route of parseRouteTable(source)) {
      ...
      let names = byPage.get(route.page);
      if (!names) {
        names = new Set();
        byPage.set(route.page, names);
      }
      names.add('query');
      for (const match of String(route.pattern).matchAll(/:([A-Za-z_$][\w$]*)/g)) {
        names.add(match[1]);
      }
    }
