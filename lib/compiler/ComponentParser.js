...
class ${name} extends AvenxComponent {
  ...
  extractTemplate(content, desBlocks, name, filePath, state, computed, methods) {
    ...
    template = this.processSlotProps(template, filePath);
    template = this.escapeScopedSlots(template, filePath);
   ...
   */
  processSlotProps(template, filePath) {
    if (!template) return template;
    const customVoidTags = [...(this.customVoidTags || []), ...getCustomVoidTags(filePath)];
    const nodes = parseHTML(template, customVoidTags);
        ...
        if (node.tagName.toLowerCase() === 'slot') {
          for (const [name, val] of Object.entries(node.attrs)) {
            if (name.startsWith(':')) {
              ...
              delete node.attrs[name];
              node.attrs[`data-props-${propName}`] = val;
            }
          }
        }
        if (node.children) {
   ...
   * @returns {string} The processed template.
   */
  escapeScopedSlots(template, filePath) {
    if (!template) return template;
    const customVoidTags = [...(this.customVoidTags || []), ...getCustomVoidTags(filePath)];
    const nodes = parseHTML(template, customVoidTags);
    ...
    const escapeTextNodes = (node) => {
      if (node.type === 'text') {
        ...
        node.children.forEach(escapeTextNodes);
      }
    };

    const findAndEscapeTemplates = (node) => {
      if (node.type === 'element') {
        if (node.tagName.toLowerCase() === 'template' && node.attrs['data-slot-props']) {
          node.children.forEach(escapeTextNodes);
        } else if (node.children) {
          node.children.forEach(findAndEscapeTemplates);
        }
      }
    };

    nodes.forEach(findAndEscapeTemplates);
    return serializeHTML(nodes, customVoidTags);
  }
