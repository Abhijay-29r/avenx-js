...
class ${name} extends AvenxPage {
    ...
    constructor(bridges, componentRegistry, props) {
        super(${JSON.stringify(state)}, ${JSON.stringify(computed)}, bridges, \`${template}\`, { ${methodStrings} }, componentRegistry, props, ${JSON.stringify(styles)}, ${JSON.stringify(resources)});
    }
}`;
    }
...
class ${name} extends AvenxComponent {
    ...
    constructor(bridges, props) {
        super(${JSON.stringify(state)}, ${JSON.stringify(computed)}, bridges, \`${template}\`, { ${methodStrings} }, props, ${JSON.stringify(styles)}, ${JSON.stringify(resources)});
    }
}`;
  }
  ...
  processForLoops(template) {
    ...
    while (true) {
      ...
      if (!innerPair) {
        logger.warn(new TemplateValidationError(AvenxErrorCodes.COMPILER_UNMATCHED_FOR_TAG).message);
        break;
      }
  ...
  processSuspense(template) {
    ...
    while (true) {
      ...
      const replacement = `<template data-ax-suspense><template data-ax-fallback>${fallbackContent}</template>${suspenseContent}</template>`;
  ...
  processErrorBoundary(template) {
    ...
    while (true) {
      ...
      const replacement = `<template data-ax-error-boundary data-ax-error-as="${errorAs}"><template data-ax-error-fallback>${fallbackContent}</template>${boundaryContent}</template>`;
      currentTemplate = currentTemplate.replace(fullMatch, replacement);
    }
    return currentTemplate;
  }
  ...
  markStaticNodes(nodes, parentIsStatic = false, inSlot = false) {
    for (const node of nodes) {
      if (node.type === 'element') {
        ...
        const nodeStatic = !currentInSlot && isStaticNode(node);
