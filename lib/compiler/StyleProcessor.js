...
function transformDeepSelectors(selector) {
  ...
  while ((matchIndex = result.search(/(?:::v-deep|:deep)\(/)) !== -1) {
    ...
    let innerContent = result.substring(openParenIdx + 1, closeParenIdx).trim();
    let suffix = result.substring(closeParenIdx + 1);
...
function scopeSelectorList(selectorList, hash) {
  ...
  return selectors
    ...
    .filter(Boolean)
    .map((selector) => {
      const scoped = selector.includes('&') ? selector.replace(/&/g, `.${hash}`) : `.${hash}${selector}`;
