...
function describe(node) {
  switch (node.type) {
    ...
    case 'Chain': return describe(node.expression);
    case 'Member':
...
function emit(node, bound, source) {
  ...
  switch (node.type) {
    ...
    case 'Chain':
      return emitChain(node.expression, bound, source);
...
/**
 * Emits a member key, refusing a forbidden one written statically.
 * @param {object} node - The Member node whose key is wanted.
 * @param {Bound|null} bound - Enclosing bindings.
 * @param {string} source - The original expression.
 * @returns {string} JavaScript source for the key.
 */
function emitMemberKey(node, bound, source) {
  if (node.computed) {
    return emit(node.property, bound, source);
  }
  if (FORBIDDEN_KEYS.has(String(node.property.value))) {
    throw new ExpressionCodegenError(
      `Access to property "${node.property.value}" is blocked for security reasons.`,
      source,
    );
  }
  return quote(node.property.value);
}

/**
 * Emits an optional chain as one expression.
 *
 * `?.` short-circuits the chain it belongs to, not merely the link that carries
 * it: when `user` is nullish, `user?.profile.name` is `undefined` because the
 * `name` read never happens. Emitting each link on its own cannot say that --
 * `axRead(axRead(user, "profile", true), "name", false)` still performs the
 * second read, on `undefined`, and throws. So a chain is emitted as one body
 * that returns at the link which short-circuits, leaving every link after it
 * unevaluated, arguments included.
 * @param {object} node - The outermost node of the chain.
 * @param {Bound|null} bound - Enclosing bindings.
 * @param {string} source - The original expression.
 * @returns {string} JavaScript source for the chain's value.
 */
function emitChain(node, bound, source) {
  const B = RUNTIME_BINDINGS;

  // Walked from the outside in, so the links come out in evaluation order with
  // the chain's root -- the part that is not itself a link -- left over.
  const links = [];
  let root = node;
  for (;;) {
    if (root.type === 'Member') {
      links.unshift({ member: root, call: null });
      root = root.object;
      continue;
    }
    if (root.type === 'Call') {
      if (root.callee.type === 'Member') {
        links.unshift({ member: root.callee, call: root });
        root = root.callee.object;
      } else {
        links.unshift({ member: null, call: root });
        root = root.callee;
      }
      continue;
    }
  ...
  const skip = (name) => `if (${name} === null || ${name} === undefined) return undefined;`;
  const steps = [`let $v = ${emit(root, bound, source)};`];

  for (const { member, call } of links) {
    if (!member) {
      if (call.optional) steps.push(skip('$v'));
      steps.push(
        `$v = ${B.call}($v, undefined, ${emitArguments(call.args, bound, source)}, ${quote(describe(call.callee))});`,
      );
      continue;
    }

    if (member.optional) steps.push(skip('$v'));
    const key = emitMemberKey(member, bound, source);
    const optional = member.optional ? 'true' : 'false';

    if (!call) {
      steps.push(`$v = ${B.read}($v, ${key}, ${optional});`);
      continue;
    }

    // The receiver is read once and used twice -- as the object of the member
    // read and as `this` -- so it is bound before either.
    const description = quote(
      member.computed
        ? `${describe(member.object)}[…]`
        : `${describe(member.object)}.${member.property.value}`,
    );
    const inner = [`const $o = $v;`, `const $f = ${B.read}($o, ${key}, ${optional});`];
    if (call.optional) inner.push(skip('$f'));
    inner.push(`$v = ${B.call}($f, $o, ${emitArguments(call.args, bound, source)}, ${description});`);
    steps.push(`{ ${inner.join(' ')} }`);
  }

  return `(() => { ${steps.join(' ')} return $v; })()`;
}
