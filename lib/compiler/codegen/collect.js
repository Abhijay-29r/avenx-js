...
function walk(nodes, expressions, statements) {
  for (const node of nodes) {
    ...
    for (const [name, value] of Object.entries(attrs)) {
      ...
      // A dynamic attribute name, `:[expr]="expr"`. Both sides are expressions
      // the string renderer evaluates at run time, and neither is an
      // interpolation, so the scans below never saw either of them. Nothing
      // reached the compiled table, and a production bundle -- which carries no
      // interpreter -- resolved both to nothing: the directive was consumed and
      // no attribute was set, silently. The same template worked in
      // development, where the interpreter is still present.
      if (name.startsWith(':[') && name.endsWith(']')) {
        add(expressions, name.slice(2, -1));
        if (typeof value === 'string' && value.includes('{{')) {
          addInterpolations(expressions, value);
        } else {
          add(expressions, value);
