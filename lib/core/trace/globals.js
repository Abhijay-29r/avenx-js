 ...
 * @param {import('./recorder.js').TraceRecorder} recorder - The recorder to log into.
 */
export function installRecordingGlobals(recorder) {
  overrides.set(
    'Date',
    buildDate(() => recorder.recordGlobal('now', Date.now())),
  );
  overrides.set(
    'Math',
    buildMath(() => recorder.recordGlobal('random', Math.random())),
  );
}
