...
/**
 * What a short-circuited link in an optional chain evaluates to.
 *
 * `?.` short-circuits the whole chain, so `user?.profile.name` has to skip the
 * `name` read rather than perform it on `undefined`. A plain `undefined` cannot
 * say which of the two happened -- `user.profile` may simply be missing, and
 * that case still throws -- so the skip carries its own value out to the
 * `Chain` node, which is where the chain ends and the value becomes
 * `undefined`. It never escapes that node.
 * @type {symbol}
 */
const CHAIN_SHORT_CIRCUIT = Symbol('avenx.optionalChain');
...
function evalNode(node, scope, frame) {
  switch (node.type) {
    ...
    case 'Chain': {
      const value = evalNode(node.expression, scope, frame);
