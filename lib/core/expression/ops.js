...
/**
 * Globals that construct code from strings.
 *
 * Refused in every context, including `<action>` bodies, because reaching them
 * is what would require `'unsafe-eval'`, and a production bundle is documented
 * to run without it.
 * @type {Set<string>}
 */
...
/**
 * Returns the host global object, or an empty object when there is none.
...
/**
 * Resolves a free identifier in an `<action>` or `<resource>` body.
 *
 * An action body is ordinary JavaScript (template-expressions.md), so a name
 * that is not component scope resolves to the page's global, exactly as it did
 * when bodies ran through `new Function` in avenx-core 0.4.x: `fetch`,
 * `window`, `setInterval`, `requestAnimationFrame` and a library a page put on
 * `window` are all reachable. Resolution order:
 *
 * 1. component scope (state, computed values, methods, bridges, `this`);
 * 2. the expression allow-list (`Date`, `Math`, …), through Trace's substitution
 *    point, so a recording still captures the non-deterministic values an
 *    action observed;
 * 3. the host global.
 *
 * Template expressions and event handlers do not use this: they keep
 * {@link readIdentifier}, which refuses browser globals by documented rule.
 * @param {object} scope - The evaluation scope.
 * @param {string} name - The identifier.
 * @returns {any} The bound value, or undefined when nothing binds it.
 */
export function readAmbientIdentifier(scope, name) {
  if (scope && name in scope) {
    return guardValue(scope[name]);
  }
  if (ALLOWED_GLOBALS.has(name)) {
  ...
  if (DYNAMIC_CODE_GLOBALS.has(name)) {
    refuse(`"${name}" constructs code from a string and cannot be used in an action body.`);
  }
 ...
 */
export function typeofAmbientIdentifier(scope, name) {
  if (scope && name in scope) {
  ...
  return typeof hostGlobal()[name];
}

/**
 * The object an assignment to a free identifier in an action body writes to.
 *
 * Component scope when it binds the name; otherwise the host global when it
 * already defines the name (`location = '/x'`, `onbeforeunload = …`);
 * otherwise component scope, which is how an assignment to an undeclared name
 * has always created a scope key. Assigning over an allow-listed global such as
 * `Math` is refused, as it is in a template.
 * @param {object} scope - The evaluation scope.
 * @param {string} name - The identifier being assigned.
 * @returns {object} The object holding the binding.
 */
export function ambientAssignmentTarget(scope, name) {
  if (scope && name in scope) {
    return scope;
  }
  if (ALLOWED_GLOBALS.has(name)) {
  ...
  return name in host ? host : scope;
}
 ...
 * @returns {any} The assigned value.
 */
export function writeAmbientIdentifier(scope, name, value) {
  ambientAssignmentTarget(scope, name)[name] = value;
  return value;
}
...
export const EXPRESSION_OPS = {
  ...
  axKey: guardKey,
  axIn: hasIn,
  axGetAmbient: readAmbientIdentifier,
  axSetAmbient: writeAmbientIdentifier,
  axTypeofAmbient: typeofAmbientIdentifier,
  axAmbientTarget: ambientAssignmentTarget,
};
