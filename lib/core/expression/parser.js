...
class Parser {
   ...
   *
   * A chain that contains a `?.` anywhere is wrapped in a `Chain` node, because
   * `?.` short-circuits the *whole* chain and not just the link that carries
   * it: `user?.profile.name` is `undefined` when `user` is nullish, rather than
   * a read of `name` on `undefined`. The wrapper marks where that short circuit
   * lands, which is the one thing the links themselves cannot say.
   * @returns {object} The AST node.
   */
  parseCallMember() {
    let node = this.parsePrimary();
    ...
    return optionalChain ? { type: 'Chain', expression: node } : node;
  }
