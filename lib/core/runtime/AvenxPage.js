...
/**
 * How many times the child-mount pass may repeat in one reconciliation.
 *
 * Each pass mounts one further level of nesting, so this is the deepest
 * component tree a page can mount in one go. Generous enough that no real tree
 * reaches it, and bounded so a component that renders itself stops.
 * @type {number}
 */
const MAX_NESTING_PASSES = 50;
...
export class AvenxPage extends AvenxComponent {
  ...
  #mountChildComponents() {
    ...
    // Mounting a child renders its template, which can introduce mount points
    // of its own -- a component nested inside a component. Those hosts do not
    // exist in the DOM while this pass is collecting them, so one pass can only
    // ever reach one level below whatever is already rendered.
    //
    // Deeper levels used to arrive by luck: a page that happened to run another
    // pass picked them up, which is why a grandchild rendered when its list was
    // in the initial state and never rendered when the same list was assigned
    // in `onMount`. The build was clean, the console silent, and the content
    // simply absent -- for the most ordinary shape there is, "fetch in onMount,
    // render a list of components that contain components".
    //
    // Repeating until a pass mounts nothing new makes the depth explicit
    // instead of accidental. Each iteration must mount at least one new child
    // to continue, so the bound is only reached by a tree nested deeper than
    // it, or by a component that renders itself.
    //
    // Reaching it is reported rather than absorbed. Stopping quietly would put
    // this back where it started: components missing from the page with
    // nothing said about it, which is the failure this whole pass exists to
    // prevent.
    for (let depth = 0; depth < MAX_NESTING_PASSES; depth += 1) {
      if (!this.#mountChildComponentsOnce(root)) return;
    }

    logger.error(formatMessage(AvenxErrorCodes.COMPONENT_NESTING_LIMIT, MAX_NESTING_PASSES));
  }

  /**
   * Runs one reconciliation pass over the page's mount points.
   * @param {object} root - The page's root element.
   * @returns {boolean} True when the pass mounted a child that was not mounted before.
   * @private
   */
  #mountChildComponentsOnce(root) {
    let mountedSomethingNew = false;
