...
export class AvenxComponent {
  ...
  /** @type {Promise<void>|null} @private */
  #unmountPromise = null;

  /** @type {boolean} @private */
  #isUnmounted = false;

  /** @type {Record<string, Element>} @private */
  #refsCache = {};
  ...
  /** @type {Record<string, import('../reactive/Resource.js').Resource>} @private */
  #resources = {};
  ...
  constructor(initialState = {}, computed = {}, bridges = {}, template = '', methods = {}, props = {}, styles = {}, resources = {}) {
    ...
    this._stateHandler = new ProxyHandlerFactory({
      ...
      getComputedValue: (key) => {
        ...
        try {
          ...
          logger.warn(formatMessage(AvenxErrorCodes.COMPUTED_EVALUTION_FAILED, key, definition, error));
          return undefined;
        } finally {
    ...
    for (const [name, handler] of Object.entries(activeResources)) {
      let handlerFn = typeof handler === 'function' ? handler : null;
      if (!handlerFn) {
        handlerFn = this.#evaluator.createMethodMap({ [name]: `return ${handler}` }, () => this.#createScope(), () => this.state)[name];
      }
      this.#resources[name] = new Resource(name, handlerFn, this);
  ...
  /**
   * Retrieves current values of all resources for scope injection.
   * @returns {object}
   * @private
   */
  #getResourceValues() {
    ...
    if (this.#resources) {
      for (const key of Object.keys(this.#resources)) {
        vals[key] = this.#resources[key].read();
      }
    }
    return vals;
  }
  ...
  #runEventHandler(compiledFnOrSource, event, slotScope = null) {
    try {
      ...
      logger.error(formatMessage(AvenxErrorCodes.EVENT_HANDLER_ERROR, sourceStr, error));
  ...
  #triggerLifecycle(hookName) {
    ...
    for (const hook of hooks) {
      try {
        ...
        logger.error(formatMessage(AvenxErrorCodes.LIFECYCLE_HOOK_ERROR, this.constructor.name, hookName, error));
  ...
  #suspend(promise) {
    ...
    let fallbackHtml = this.#template;
    let hasSuspense = false;
    
    fallbackHtml = fallbackHtml.replace(
      /<template data-ax-suspense><template data-ax-fallback>([\s\S]*?)<\/template>[\s\S]*?<\/template>/gi,
      (match, fallback) => {
        hasSuspense = true;
  ...
  #handleComponentError(error) {
    ...
    fallbackHtml = fallbackHtml.replace(
      /<template data-ax-error-boundary data-ax-error-as="([^"]*)"><template data-ax-error-fallback>([\s\S]*?)<\/template>[\s\S]*?<\/template>/gi,
      (match, asName, fallback) => {
