...
export class AvenxComponent {
  ...
  constructor(initialState = {}, computed = {}, bridges = {}, template = '', methods = {}, props = {}, styles = {}) {
    ...
    this._mixinProps = {};
    const reservedKeys = ['state', 'data', 'methods', 'computed', 'props', 'styles', 'onBeforeMount', 'onMount', 'onBeforeUpdate', 'onUpdate', 'onUnmount', 'onActivate', 'onDeactivate'];
    for (const mixin of globalMixins) {
   ...
   * @private
   */
  #runEventHandler(compiledFnOrSource, event) {
    try {
      if (typeof compiledFnOrSource === 'function') {
        const scope = this.#createScope(this.#methods, { event });
      ...
      return this.#evaluator.executeStatement(compiledFnOrSource, this.#createScope(this.#methods, { event }), this.state);
    } catch (error) {
  ...
  __updateTranscludedContent(virtualChildNodes) {
    ...
    if (this.#element) {
      ...
      slots.forEach((slotEl) => {
        ...
        this.#patcher.patchElement(slotEl, newSlotWrapper, slotEvaluator);
      });
    }
  }
  ...
  #reportError(error, origin) {
    ...
    if (app && typeof app._handleError === 'function') {
      app._handleError(error, this, origin);
    }
  }
