...
/**
 * Method names that collide with a real AvenxComponent instance method.
 *
 * An action or method with one of these names shadows framework machinery --
 * `update` is the render method, `destroy` tears the component down -- so it is
 * a genuine mistake and warns (AVX_W26).
 * @type {string[]}
 */
export const RESERVED_INSTANCE_METHOD_KEYS = [
  'mount',
  'unmount',
  'update',
  'destroy',
 ...
 *
 * These are the documented, supported way to define a hook: an
 * `<action name="onMount">` is looked up by {@link AvenxComponent#triggerLifecycle}
 * and run at the right moment. They are meant to be declared, so they do not
 * warn -- unlike a collision with a reserved instance method.
 * @type {string[]}
 */
...
/**
 * Every name the runtime treats specially: reserved instance methods and
 * lifecycle hooks. Retained for callers that need the full set; the collision
 * warning uses {@link RESERVED_INSTANCE_METHOD_KEYS} alone.
 * @type {string[]}
 */
export const RESERVED_INSTANCE_KEYS = [...RESERVED_INSTANCE_METHOD_KEYS, ...LIFECYCLE_HOOK_KEYS];
...
export class AvenxComponent {
  ...
  constructor(initialState = {}, computed = {}, bridges = {}, template = '', methods = {}, props = {}, styles = {}, resources = {}, options = {}) {
    ...
    for (const [name, fn] of Object.entries(this.#methods)) {
      if (RESERVED_INSTANCE_METHOD_KEYS.includes(name)) {
        logger.warn(
          formatMessage(
   ...
   *
   * A component does not own mount points -- {@link AvenxPage} does, for its
   * whole subtree -- but a component's template can still carry them, and when
   * its own props change it re-renders and writes new values onto its
   * children's host elements. Discarding the notification here meant nothing
   * reconciled those values onto the child *instances*, so a prop stopped
   * propagating one level below the page:
   *
   *     page -> Lvl1 (props updated) -> Lvl2 (host updated, instance stale)
   *
   * Every level below that kept its first value for the life of the page. The
   * notification is therefore passed to whoever owns the mount points, which is
   * the page at the top of `$parent`. The page coalesces it to one pass per
   * flush, so a chain converges a level per flush rather than re-entering here.
   * @returns {void}
   */
  __onChildPropsChanged() {
    const parent = this.$parent;
    if (parent && typeof parent.__onChildPropsChanged === 'function') {
      parent.__onChildPropsChanged();
    }
  }
   ...
   *
   * `this.$router.navigate(hash)` is how the routing documentation says to
   * navigate from inside a component, and the routing guide documents
   * `back()`, `forward()` and `go(delta)` on the same accessor -- but nothing
   * in the runtime ever defined `$router`, so every one of those examples threw
   * `Cannot read properties of undefined (reading 'navigate')`. The router was
   * only reachable as `this.$app.router`, which no document mentions.
   *
   * Falls back to the active router registered on the window when the component
   * has no `$app` -- a page mounted directly still has a router to talk to, and
   * that is the same registry `$route` above reads.
   * @returns {object|null} The router, or null when the application has none.
   */
  get $router() {
