 ...
 * @property {string} ATLAS_NO_REACHABLE_PAGE - AVX_W53: The application compiles pages but declares no route and mounts none, so it renders nothing.
...
export const AvenxErrorMessages = {
  ...
  [AvenxErrorCodes.ATLAS_NO_REACHABLE_PAGE]:
    'This application compiles {0} page(s) but declares no route and mounts none, so it renders nothing: {1}.\nAn AvenxApp that is constructed and never told what to show produces an empty container and no error, in the browser or here. Call app.initRouter({ \'\': \'{2}\' }) in {3}, or app.mountPage(\'{4}\') to mount one directly. Silence this class with "warnings": { "AVX_W53": "off" } in avenx.config.json.',
