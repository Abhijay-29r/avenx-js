 ...
 * @property {string} COMPILER_EXPRESSION_NOT_COMPILED - AVX_W48: An expression could not be compiled to a closure and will be interpreted at runtime.
...
export const AvenxErrorMessages = {
  ...
  [AvenxErrorCodes.COMPILER_EXPRESSION_NOT_COMPILED]:
    '{0} expression(s) could not be compiled to executable code and will be interpreted at runtime:\n{1}\nThose expressions keep the runtime expression parser in the bundle. Rewrite them in the supported expression language, or move the logic into an <action>. Silence this class with "warnings": { "AVX_W48": "off" } in avenx.config.json.',
