  ...
  mountPage(name, params = {}, options = {}) {
    ...
    if (this.#target) {
      ...
      for (const comp of toUnmount) {
        if (comp === activeLayout && comp.$keepAlive && this.#pageCache && activeLayoutName) {
      ...
      const proceed = () => {
        ...
        const setupPage = (PClass, pName, pParams, mTarget, isLayout) => {
          let inst;
          const isCached = isLayout && this.#pageCache && this.#pageCache.has(pName);
          if (isCached) {
        ...
        if (LayoutClass && !isSameLayout) {
          ...
          setupPage(PageClass, name, params, this.#target, false);
        }
