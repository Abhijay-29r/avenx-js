...
import { requireStringRenderer } from '../renderer/stringRenderer.js';
...
export class AvenxApp {
  ...
  constructor(config) {
    ...
    // Built-ins are adopted from the registry rather than imported here. A
    // direct import made every application carry the virtual list, the template
    // renderer and the DOM patcher whether or not it wrote `<VirtualList>`; the
    // compiler now adds the registering module when it sees the tag.
    for (const [name, componentClass] of getBuiltins()) {
  ...
  /**
   * Invokes all registered error handlers safely, and reports what none claimed.
   *
   * This method used to be the loop alone. An application that registers no
   * handler is the default -- `avenx init` writes no `onError` call and nothing
   * requires one -- so the loop body never ran and the error was discarded.
   * What the developer saw was a component that mounted and rendered nothing:
   * no exception, no console output, no warning. A rendering path can break
   * completely and look exactly like an application with nothing to show.
   *
   * So an error that reaches here unclaimed is reported. `logger.error` rather
   * than a rethrow, deliberately: this is called from inside a `catch` that has
   * already decided the component cannot render, and throwing again from there
   * would tear down the rest of the flush -- turning one blank component into a
   * blank page. Reporting keeps the failure local and visible.
   *
   * "Unclaimed" is the precise condition. An application that registered a
   * handler has said it will deal with its own errors, and an
   * `onErrorCaptured` hook that stopped propagation never reaches this method
   * at all.
   ...
   * @param {boolean} [logged] - Whether the call site already reported this
   *   error with a diagnostic of its own. A failing lifecycle hook is already
   *   AVX_R12 and a failing handler already AVX_R09; repeating them as AVX_R33
   *   would print the same failure twice under two codes.
   * @private
   */
  _handleError(error, component, origin, logged = false) {
    if (this.#errorHandlers.length === 0 && !logged) {
      logger.error(
        formatMessage(
          AvenxErrorCodes.COMPONENT_RENDER_ABORTED,
          (component && component.constructor && component.constructor.name) || 'component',
          origin || 'render',
  ...
  registerPage(name, pageClass) {
    ...
    // Only a *different* class is an overwrite. The compiler auto-registers
    // every page under src/pages, and an application may register the same
    // page again by hand -- the routing tutorial tells the reader to do exactly
    // that, and the result was four AVX_W07 warnings in the console on every
    // page load, each claiming an overwrite that does not happen. Re-registering
    // the identical class changes nothing, so there is nothing to report.
    if (this.pages.has(name) && this.pages.get(name) !== pageClass) {
   ...
   *
   * Replacing an existing router destroys it first. Without that, each call
   * left the previous router subscribed to navigation events and registered
   * with the delegate: the listeners accumulated one per call, and every
   * abandoned router went on handling hash changes and mounting pages into the
   * same target, so a second navigation ran the route twice. Re-initialising
   * routes is ordinary -- a hot reload does it, and so does an application that
   * installs different routes once a session is known.
   ...
   * @param {object} bridgeData - A bridge() instance or a plain reactive object.
   */
  registerBridge(name, bridgeData) {
    ...
    const reactiveState = new Proxy(bridgeData, handlerFactory.create());
    ...
    if (this.#target) {
      ...
      if (targetToUpdate) {
        ...
        for (const [key, val] of Object.entries(params)) {
          if (targetToUpdate.params[key] !== val) {
            paramsChanged = true;
          }
        ...
        // A navigation that changed the route parameters re-renders the page it
        // lands on, whether or not the page's template reads any of them: its
        // lifecycle hooks are part of the contract, and onUpdate must run for
        // the route now being shown. This used to happen implicitly, because
        // every state write scheduled a render regardless of what observed it.
        // Now that writes nothing depends on are skipped, the navigation has to
        // say so -- and only when something actually changed, or a re-entry on
        // identical parameters would start reporting an update that never used
        // to happen.
        if (paramsChanged && typeof targetToUpdate.scheduleUpdate === 'function') {
          targetToUpdate.scheduleUpdate();
        }
      ...
      const proceed = () => {
        ...
        if (transitionName) {
          // Route transitions are the router's only use of the patcher, and a
          // transition is a template construct the IR does not lower yet -- so
          // a build that has one has the string renderer anyway. Asking the
          // registry keeps the router from being the reason an application
          // that uses neither still carries 87 KB of it.
          const patcher = new (requireStringRenderer().DomPatcher)();
          if (exitWrapper) {
