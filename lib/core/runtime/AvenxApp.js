...
export class AvenxApp {
  ...
  #errorHandlers = [];
  /** @type {Set<any>} @private */
  #installedPlugins = new Set();
  /** @type {LruCache|null} @private */
  #pageCache = null;
   ...
   * @param {object | Function} plugin - The plugin to install.
   ...
   */
  get activePage() {
    return this.#activeComponents[0] || null;
  }
  ...
  mountPage(name, params = {}, options = {}) {
    ...
    if (this.#target) {
      ...
      const activePageName = activePage ? activePage.$pageName : null;
      ...
      // Handle current page cleanup or caching
      let unmountPromise = null;
      if (activePage) {
        if (activePage.$keepAlive && this.#pageCache && activePageName) {
          // Deactivate and cache
          ...
          activePage.__setMountTarget(domFragment, true);

          if (typeof activePage.onDeactivate === 'function') {
            try {
          ...
          this.#pageCache.set(activePageName, {
            pageInstance: activePage,
            domFragment: domFragment
          });
        } else {
          if (typeof activePage.unmount === 'function') {
            const res = activePage.unmount();
      ...
      const proceed = () => {
        ...
        let pageInstance;
        const isCached = this.#pageCache && this.#pageCache.has(name);

        if (isCached) {
          const cached = this.#pageCache.get(name);
          this.#pageCache.delete(name);
          pageInstance = cached.pageInstance;
          const domFragment = cached.domFragment;

          this.#target.appendChild(domFragment);
          pageInstance.__setMountTarget(this.#target, true);
          ...
          if (typeof pageInstance.onActivate === 'function') {
            try {
              pageInstance.onActivate(params);
            } catch (err) {
              logger.error('Error in onActivate hook:', err);
            }
          }
        } else {
          pageInstance = new PageClass(this.bridges, this.components);
          pageInstance.$app = this;
          pageInstance.$pageName = name;
          pageInstance.$keepAlive = !!options.keepAlive;
