    ...
    this.#runGuards(guards, to, from)
      .then((result) => {
        ...
        if (result === false) {
          ...
          this.redirectContext = null;
          if (tracer.on) {
            // Recorded where the navigation is committed rather than where it
            // is requested, so guard-cancelled and redirected attempts do not
            // appear in the trace as navigations that happened. Replay drives
            // navigations through the router's own delegate, so a recorded
            // navigation reproduces guards, redirects and page mounting rather
            // than bypassing them.
