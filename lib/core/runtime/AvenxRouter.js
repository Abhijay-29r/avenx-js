...
import { GuardContext } from './AvenxGuard.js';
...
export class AvenxRouter {
  ...
  constructor(app, routes = {}, options = {}) {
    ...
    // AVX_W08 used to be raised here for any pattern that did not begin with a
    // slash once its '#' was stripped. That was a true warning about a real
    // failure -- such a pattern could never match -- but it named the symptom
    // rather than the cause, and it fired on `''`, which is what this project's
    // own scaffold writes. RouteMatcher.normalizePattern now brings every
    // spelling into the form the matcher compares against, so there is no
    // longer a pattern shape that silently fails to resolve. The code stays
   ...
   *
   * Idempotent. Removing an event listener that is no longer attached is a
   * no-op in a real DOM, so a second call was harmless in a browser -- but it
   * also re-ran `delegate.destroy()` and `unregisterRouter`, which is not
   * something a teardown path should depend on being harmless. The router is
   * destroyed once and then knows it.
   * @returns {void}
   */
  destroy() {
    ...
    this.unsubscribeHashChange = null;
    this.unsubscribeLinkClick = null;
   ...
   */
  /**
   * Builds the capabilities handed to a guard for one navigation.
   *
   * A guard used to receive nothing but the route, which made "is this visitor
   * signed in?" unanswerable: it had no supported route to shared state. The
   * context carries the application's bridges and the router asking, and
   * deliberately carries no browser globals -- a guard that needs one reads it
   * through a bridge, like everything else.
   * @returns {GuardContext} The context for this navigation.
   * @private
   */
  #guardContext() {
    return new GuardContext({
      bridges: this.app ? this.app.bridges : {},
      app: this.app || null,
      router: this,
    });
  }
    ...
    return new Promise((resolve, reject) => {
      const nextGuard = (index) => {
        if (index >= guards.length) {
          ...
          // Built per guard rather than per navigation so a guard that runs
          // after an earlier one mutated a bridge sees the current registry.
          const context = this.#guardContext();
          try {
            if (typeof Guard === 'function') {
              if (Guard.prototype && typeof Guard.prototype.canActivate === 'function') {
                const instance = new Guard(context);
                canActivateResult = instance.canActivate(to, from, context);
              } else {
                canActivateResult = Guard(to, from, context);
              }
            } else if (Guard && typeof Guard.canActivate === 'function') {
              canActivateResult = Guard.canActivate(to, from, context);
            } else {
      ...
      schedule(() => {
        ...
        const pageRoot =
          ...
          (typeof document.querySelector === 'function'
            ? document.querySelector('[data-ax-page]') || document.querySelector('main') || document.body
            : document.body || null);
