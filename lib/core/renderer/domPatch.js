...
export class DomPatcher {
  ...
  #applyDirectives(el, resolveExpression, app) {
    ...
    // 4. data-ax-style
    if (el.hasAttribute('data-ax-style')) {
      const expr = el.getAttribute('data-ax-style');
      try {
        const value = localResolve(expr);
        const newStyles = {};

        const isUnsafeStyleValue = (str) => /expression\(|javascript:/i.test(str);

        const camelToKebab = (key) => key.replace(/([A-Z])/g, '-$1').toLowerCase();

        const mergeStyleObject = (obj) => {
          if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return;
          for (const [key, raw] of Object.entries(obj)) {
            if (raw == null || raw === false) continue;
            const prop = camelToKebab(key);
            const str = String(raw);
            if (isUnsafeStyleValue(str)) continue;
            newStyles[prop] = str;
          }
        };
        ...
        if (typeof value === 'string') {
          for (const part of value.split(';')) {
            ...
            if (!prop) continue;
            if (isUnsafeStyleValue(raw)) continue;
            newStyles[prop] = raw;
          }
        } else if (Array.isArray(value)) {
          for (const item of value) {
            mergeStyleObject(item);
          }
        } else if (value && typeof value === 'object') {
          mergeStyleObject(value);
        }

        // Clean up styles added by previous data-ax-style evaluation
        if (el.__lastAxStyles) {
          for (const prop of el.__lastAxStyles) {
        ...
        el.__lastAxStyles = Object.keys(newStyles);
      } catch (err) {
        logger.warn(
          formatMessage(AvenxErrorCodes.DIRECTIVE_STYLE_EVALUATION_FAILED, expr, err.message || err)
        );
      }
    }
