...
function wrapValue(val) {
  ...
  const traps = {
    ...
    get(target, key, receiver) {
      ...
      // Never hand out a dynamic-code constructor, however it was reached.
  ...
  if (typeof val === 'function') {
     ...
     * @param {object} target - The target function.
     * @param {any[]} argumentsList - The arguments passed.
     * @param {Function} newTarget - The constructor originally invoked.
     * @returns {any}
     */
    traps.construct = function (target, argumentsList, newTarget) {
      assertNotFunctionConstructor(target);
      const rawArgs = argumentsList.map(unwrap);
      return wrapValue(Reflect.construct(target, rawArgs, newTarget === undefined ? target : newTarget));
    };
  }
