...
/**
 * Renders one causation entry.
 *
 * Entries are recorded as `{target, key}` rather than as a formatted path: the
 * path walks the parent chain and joins strings, and it was built on *every*
 * write for a diagnostic that fires only when a cycle is detected. A watcher
 * name is already a string and is kept as one.
 * @param {object|string} entry - A recorded entry.
 * @returns {string} The property path or watcher label.
 */
function formatCausationEntry(entry) {
  return typeof entry === 'string' ? entry : getPropertyPath(entry.target, entry.key);
}
...
export function getActiveCausationTrace() {
  return causationTrace.map(formatCausationEntry);
}
...
export function trigger(target, key, oldValue, newValue) {
  ...
  if (triggerDepth > MAX_TRIGGER_DEPTH) {
    const cycleStr = causationTrace.slice(-6).map(formatCausationEntry).join(' -> ') || 'synchronous-watcher-cycle';
    logger.error(
      formatMessage(
  ...
  try {
    ...
    for (const k of keys) {
      if (typeof k !== 'symbol') {
        causationTrace.push({ target, key: k });
      }
    }
        ...
        if (watchers) {
          ...
          for (const watcher of toRun) {
            if (activeUpdatingWatchers.has(watcher)) {
              const cycleStr = `${watcher.name || 'anonymous'} -> ${causationTrace.map(formatCausationEntry).join(' -> ')} -> ${watcher.name || 'anonymous'}`;
              logger.error(
                formatMessage(
                  AvenxErrorCodes.REACTIVE_DEADLOCK_DETECTED,
...
export class AvenxWatcher {
  ...
  addDep(target, key, watchersSet) {
    // Keyed by the watcher set rather than scanned for it. A render effect
    // reads every value it displays, so a component with a few hundred
    // bindings made this a linear scan per read -- quadratic in the number of
    // reads, on the hot path of every evaluation.
    if (!this.deps.has(watchersSet)) {
      this.deps.set(watchersSet, { target, key, watchersSet });
    }
  }
   ...
   */
  cleanupDeps(oldDeps) {
    for (const [watchersSet] of oldDeps) {
      if (!this.deps.has(watchersSet)) {
        watchersSet.delete(this);
      }
    }
  }
  ...
  update() {
    ...
    if (activeUpdatingWatchers.has(this)) {
      const cycleStr = `${this.name} -> ${causationTrace.map(formatCausationEntry).join(' -> ')} -> ${this.name}`;
      logger.error(
        formatMessage(
          AvenxErrorCodes.REACTIVE_DEADLOCK_DETECTED,
          ' (synchronous watcher cycle)',
  ...
  teardown() {
    ...
    for (const [watchersSet] of this.deps) {
      watchersSet.delete(this);
    }
    this.deps.clear();
  }
