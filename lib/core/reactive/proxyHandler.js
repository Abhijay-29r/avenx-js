...
export class ProxyHandlerFactory {
  ...
  get(target, key, receiver) {
    ...
    // Computed properties belong to the component's own state object, not to
    // every object reachable through it. One handler is shared by the root and
    // every nested proxy it creates, so an unguarded check made each nested
    // object claim all of the component's computeds: `state.query.total`
    // returned the component's `total`, and `Object.keys(state.query)` listed
    // it. See `isRootTarget`.
    if (this.isRootTarget(target) && this.computedKeys.has(key)) {
  ...
  /**
   * Whether `target` is the state object the computed properties belong to.
   *
   * One handler instance serves a component's root state object and every
   * nested object reached through it, because they share this factory. The
   * computed traps therefore have to ask which object they are answering for:
   * without this, every nested object reported and resolved the component's
   * computed properties as its own. `Object.keys(state.form)` listed them,
   * `{ ...state.form }` copied them, `JSON.stringify(state.query)` serialised
   * them into a request, and `state.query.total` returned a number that has
   * nothing to do with the query string.
   *
   * `rootTarget` is recorded by the trap wrappers on first access, and a nested
   * object can only be reached through the root, so it is always set by the
   * time a nested target arrives here. When it somehow is not, the target is
   * treated as the root -- the previous behaviour, which is correct for the
   * root and is the only object that can be seen first.
   * @param {object} target - The object a trap is answering for.
   * @returns {boolean} True when `target` owns the computed properties.
   * @private
   */
  isRootTarget(target) {
    return !this.rootTarget || target === this.rootTarget;
  }
