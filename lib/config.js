...
function loadConfig(baseDir) {
  const defaults = {
    ...
    trace: {
      /**
       * Property paths whose values are withheld from every recording.
       *
       * Applied at record time, so a matched value never enters the buffer and
       * cannot leak through an export path that forgot to strip it. Supports
       * `auth.token`, `user.*`, `*.password` and `secrets.**`.
       */
      redact: [],
      /** Ring-buffer capacity. Zero means the recorder's own default. */
      maxNodes: 0,
    },
  };
