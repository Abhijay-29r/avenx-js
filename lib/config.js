 ...
 * unrecognised.
 * @type {{topLevel: string[], server: string[], style: string[], debug: string[], logging: string[], hooks: string[], trace: string[], rewind: string[]}}
 */
export const CONFIG_SCHEMA = {
  ...
  rewind: ['onConflict', 'maxSnapshotItems'],
...
function loadConfig(baseDir) {
  const defaults = {
    ...
    // Off by default. Reusing a previous compilation is only safe while the key
    // covers everything a unit was derived from, and the test that demonstrates
    // that -- an edit sequence whose incremental output is byte-identical to a
    // cold build -- is younger than the code it checks. A project opts in; a
    // project that does not is unaffected.
    incremental: false,
    server: {
      port: 3000,
      host: 'localhost',
  ...
  try {
    ...
    // templateGlobals holds identifier names, not values, and JavaScript
    // identifiers may start with `$` — which env interpolation would read as a
    // placeholder and blank out, making a name like `$i18n` impossible to
    // declare. Nothing in this option could ever want to be an environment
    // reference, so it is taken verbatim from the file.
    if (userConfigRaw && Array.isArray(userConfigRaw.templateGlobals)) {
      userConfig.templateGlobals = userConfigRaw.templateGlobals;
    }
    ...
    if (userConfig && typeof userConfig === 'object' && !Array.isArray(userConfig)) {
      ...
      for (const key of Object.keys(userConfig)) {
        if (!allowedTopLevel.includes(key)) {
          ...
          if (key === 'rewind' && userConfig.rewind && typeof userConfig.rewind === 'object' && !Array.isArray(userConfig.rewind)) {
            const allowedRewindKeys = CONFIG_SCHEMA.rewind;
            for (const subKey of Object.keys(userConfig.rewind)) {
              if (!allowedRewindKeys.includes(subKey)) {
                const closest = getClosestKey(subKey, allowedRewindKeys);
                const suggestion = closest ? `. Did you mean "rewind.${closest}"?` : '.';
                logger.warn(`Unknown configuration option "rewind.${subKey}" in avenx.config.json${suggestion} Supported options for "rewind" are: ${allowedRewindKeys.join(', ')}.`);
              }
            }
          }
    ...
    // Identifiers a plugin publishes into every component's template scope —
    // `app.mixin()` members, most often. Template validation has no way to see
    // those: they are contributed at runtime, by code the compiler never reads.
    // Declaring them here is how a project says "this name is provided", so a
    // template using it is checked rather than either warned about wrongly or
    // exempted by switching the whole check off.
    if (
      !Array.isArray(config.templateGlobals) ||
      config.templateGlobals.some((name) => typeof name !== 'string' || name.trim() === '')
    ) {
      throw new Error('templateGlobals must be an array of non-empty identifier names');
    }
    ...
    const allowedConflictPolicies = ['safe', 'force', 'abort'];
    if (!allowedConflictPolicies.includes(config.rewind.onConflict)) {
      throw new Error(
        `Invalid rewind.onConflict "${config.rewind.onConflict}". Allowed values: ${allowedConflictPolicies.map((p) => `"${p}"`).join(', ')}`,
      );
    }
    if (
      typeof config.rewind.maxSnapshotItems !== 'number' ||
      !Number.isInteger(config.rewind.maxSnapshotItems) ||
      config.rewind.maxSnapshotItems <= 0
    ) {
      throw new Error('rewind.maxSnapshotItems must be a positive integer');
    }
