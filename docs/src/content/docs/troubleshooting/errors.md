...
| `[AVX_C26]` | Malformed template in <{0}>: {1} | **Identifier:** `COMPILER_MALFORMED_TEMPLATE`.<br />**Cause:** A tag is never closed with `>` (usually an attribute quote that never closes) or a `<!--` comment has no `-->`. Everything after it would be read as part of that tag or comment.<br />**Resolution:** Close the tag, quote or comment at the location shown. |
